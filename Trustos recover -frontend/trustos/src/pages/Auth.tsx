...
export function VerifyPage() {
  ...
  async function submit(event: FormEvent) {
    event.preventDefault(); setError('');
    if (!/^\d{6}$/.test(code)) { setError('Enter the six-digit code shown above.'); return; }
    setBusy(true);
  ...
  return <AuthLayout><div className="auth-form-wrap"><div className="auth-form-heading"><span className="eyebrow">ACCOUNT SETUP / 02</span><div className="auth-step-counter">STEP 02 OF 02</div><h1>Confirm your account.</h1><p>Enter the code shown below for <strong>{email || 'your account'}</strong> to access the workspace.</p></div>
    
    {demoCode ? <div className="dev-code-card"><span>YOUR ACCESS CODE</span><strong aria-label={`Access code ${demoCode.split('').join(' ')}`}>{demoCode}</strong><small>Shown here, not sent by email. Valid for 10 minutes.</small></div> : <InlineNotice type="warning">No code is available in this tab. Log in to return to this step, or create an account.</InlineNotice>}
    {error && <InlineNotice type="error">{error}</InlineNotice>}
    <form className="auth-form verify-form" onSubmit={submit}><div className="form-field"><label htmlFor="verify-code">Six-digit code</label><div className="input-shell"><input id="verify-code" className="verification-input" type="text" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} placeholder="000000" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, '').slice(0, 6))} required /></div></div><button className="btn btn-primary btn-submit" type="submit" disabled={busy || !email}>{busy ? 'Confirming…' : 'Confirm and continue'}<ArrowRight size={18} aria-hidden="true" /></button></form>
    <div className="verify-actions"><button type="button" onClick={requestNewCode} disabled={busy || seconds > 0 || !email}>Generate another code {seconds > 0 ? `in ${seconds}s` : ''}</button><Link to="/signup">Change email</Link></div>
...
export function ForgotPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!result.devCode) { setError('We couldn’t find an account for that email. Check the address or create an account. No email was sent.'); return; }
      saveReset(result.email, result.devCode);
      navigate(`/reset-password?email=${encodeURIComponent(result.email)}`);
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to prepare a reset code.'); }
    finally { setBusy(false); }
  }
  return <AuthLayout><div className="auth-form-wrap"><div className="auth-form-heading"><span className="eyebrow">ACCOUNT ACCESS</span><h1>Reset your password.</h1><p>Enter the email address for your account. If it matches, your code will appear on the next screen; no email is sent.</p></div>
    {error && <InlineNotice type="error">{error}</InlineNotice>}
...
export function ResetPage() {
  ...
  if (done) return <AuthLayout><div className="auth-form-wrap"><div className="auth-success-icon"><Check size={26} /></div><span className="eyebrow">PASSWORD RESET</span><h1>Password updated.</h1><p>Your password has been updated for this account. Log in to continue; earlier sessions were closed.</p><Link className="btn btn-primary btn-submit" to="/login">Go to log in <ArrowRight size={18} /></Link></div></AuthLayout>;
  return <AuthLayout><div className="auth-form-wrap"><div className="auth-form-heading"><span className="eyebrow">ACCOUNT ACCESS / RESET</span><h1>Choose a new password.</h1><p>Enter the code and a new password for <strong>{email || 'your account'}</strong>.</p></div>
    
    {demoCode && <div className="dev-code-card"><span>YOUR RESET CODE</span><strong>{demoCode}</strong><small>Shown here, not sent by email. Valid for 10 minutes.</small></div>}
    {error && <InlineNotice type="error">{error}</InlineNotice>}
    <form className="auth-form" onSubmit={submit}><Field id="reset-code" label="Six-digit code" value={code} onChange={(s) => setCode(s.replace(/\D/g, '').slice(0, 6))} placeholder="000000" autoComplete="one-time-code" maxLength={6} /><Field id="reset-password" label="New password" type="password" value={password} onChange={setPassword} placeholder="At least 8 characters" autoComplete="new-password" /><Field id="reset-confirm" label="Confirm new password" type="password" value={confirm} onChange={setConfirm} placeholder="Repeat the new password" autoComplete="new-password" /><button className="btn btn-primary btn-submit" type="submit" disabled={busy || !email}>{busy ? 'Updating password…' : 'Update password'}<ArrowRight size={18} /></button></form>
