...
export function createLocalAuthHandler() {
  return async (req, res) => {
    ...
    if (req.method !== 'POST') return fail(res, 404, 'NOT_FOUND', 'This page could not be found.');
    ...
    if (pathname === '/api/auth/signup') {
      ...
      if (data.termsAccepted !== true) return fail(res, 400, 'TERMS_REQUIRED', 'Please accept the Terms & Conditions to continue.');
    ...
    if (pathname === '/api/auth/login') {
      ...
      if (!account.verifiedAt) return fail(res, 403, 'EMAIL_UNVERIFIED', 'Please confirm your account code first.', { email, devCode: account.code });
    ...
    if (pathname === '/api/auth/verify') {
      ...
      if (!account.code || account.codeExpires < Date.now()) return fail(res, 400, 'CODE_EXPIRED', 'This code has expired. Request a new one.');
      if (code !== account.code) return fail(res, 400, 'CODE_INVALID', 'That code does not match. Please check the code shown on this page.');
    ...
    if (pathname === '/api/auth/password/reset') {
      ...
      if (!account?.resetCode || account.resetExpires < Date.now() || String(data.code ?? '').trim() !== account.resetCode) {
        return fail(res, 400, 'INVALID_RESET_CODE', 'That reset code is invalid or has expired. Request another code.');
      }
    ...
    return fail(res, 404, 'NOT_FOUND', 'This page could not be found.');
  };
}
