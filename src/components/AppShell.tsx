...
export function AppShell() {
  ...
  async function signOut() {
    ...
    catch { setLogoutError('Could not log out. Please retry.'); }
  }
  return <div className="app-shell">
    ...
    <aside id="app-navigation" className={`app-sidebar ${menuOpen ? 'app-sidebar--open' : ''}`} aria-label="Workspace navigation"><div className="app-sidebar-top"><Brand to="/app" /><button className="app-mobile-close" type="button" aria-label="Close navigation" onClick={() => setMenuOpen(false)}><X size={20} /></button></div>
      <div className="app-sidebar-nav"><span className="app-nav-label">YOUR WORKSPACE</span><nav>{navigation.map(({ path, label, icon: Icon, end }) => <NavLink key={path} to={path} end={end} className={({ isActive }) => `app-nav-link ${isActive ? 'app-nav-link--active' : ''}`}><Icon size={19} strokeWidth={1.7} aria-hidden="true" /><span>{label}</span></NavLink>)}</nav></div>
      <div className="app-sidebar-bottom"><div className="app-help-box"><CircleHelp size={20} strokeWidth={1.6} aria-hidden="true" /><span>Need context before you begin?</span><Link to="/app/assistant">Ask Trustos AI <ArrowUpRight size={14} aria-hidden="true" /></Link></div><div className="app-user"><div className="app-user-avatar">{user?.name.charAt(0).toUpperCase()}</div><div className="app-user-text"><strong>{user?.name}</strong><span>{user?.email}</span></div><button type="button" title="Log out" aria-label="Log out" onClick={signOut}><ArrowLeftFromLine size={18} /></button></div>{logoutError && <small className="app-logout-error" role="alert">{logoutError}</small>}</div>
    </aside>
    <div className="app-main-wrap"><header className="app-topbar"><div className="app-topbar-left"><button ref={menuButton} type="button" className="app-menu-btn" aria-controls="app-navigation" aria-label="Open navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen(true)}><Menu size={22} /></button><Link to="/app" className="app-mobile-mark" aria-label="Trustos workspace"><img src="/brand/linuxboss-mark.webp" alt="" /></Link><div className="app-breadcrumb"><span>WORKSPACE</span><ChevronRight size={14} aria-hidden="true" /><strong>{current.toUpperCase()}</strong></div></div><div className="app-topbar-actions"><ThemeToggle small /><Link className="topbar-user" to="/app/account" aria-label="Account settings">{user?.name.charAt(0).toUpperCase()}</Link></div></header>
      <div className="app-page"><Outlet /></div><footer className="app-footer"><span>TRUSTOS / LINUXBOSS</span><Link to="/">Back to site <ArrowUpRight size={13} aria-hidden="true" /></Link></footer>
    </div>
  </div>;
}
