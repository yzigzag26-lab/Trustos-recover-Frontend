...
try {
  ...
  assert.deepEqual(Object.keys(auth.authService).sort(), ['login', 'logout', 'requestPasswordReset', 'resetPassword', 'session', 'signup']);
  for (const [operation, args] of [
    ['session', []], ['signup', ['Test', 'nobody@example.invalid', 'unused', true]],
    ['login', ['nobody@example.invalid', 'unused']],
    ['requestPasswordReset', ['nobody@example.invalid']],
    ['resetPassword', ['unused']], ['logout', []],
  ]) {
  ...
  console.log('✓ signup retains consent UI but cannot create an account or session');
  ...
  await page.getByText(/When email confirmation is connected, we will send a verification link to/).waitFor();
  await page.getByText('nobody@example.invalid').waitFor();
  assert.equal(await page.locator('.auth-form input').count(), 0, 'Email-link screen must not ask for a numeric code.');
  assert(await page.getByRole('button', { name: 'Resend verification email' }).isDisabled(), 'Resend must not pretend to send email.');
  ...
  await page.locator('.inline-notice--error').getByText('Authentication is not connected yet. No request was sent.').waitFor();
  assert((await page.url()).endsWith('/forgot-password'), 'An unconnected email request cannot navigate directly to the reset form.');
  await page.goto(base + '/reset-password');
  await page.getByRole('heading', { name: 'Choose a new password.' }).waitFor();
  assert.equal(await page.locator('.auth-form input').count(), 2, 'Link-based reset shell should have password fields only.');
  await page.getByLabel('New password', { exact: true }).fill('NotARealNewPassword123!');
  await page.getByLabel('Confirm new password').fill('NotARealNewPassword123!');
  await page.getByRole('button', { name: 'Update password' }).click();
  await page.locator('.inline-notice--error').getByText('Authentication is not connected yet. No request was sent.').waitFor();
  assert.equal(await page.getByRole('heading', { name: 'Password updated.' }).count(), 0);
  console.log('✓ login, email-link confirmation, and password-only reset remain unavailable without invented delivery or results');
  ...
  await page.getByRole('heading', { name: 'Email links' }).waitFor();
  await page.getByText(/This version does not send those links/).waitFor();
  console.log('✓ workspace remains protected; Terms and privacy describe future email links honestly');

  for (const width of [900, 390, 320]) {
    await page.setViewportSize({ width, height: 844 });
    for (const route of ['/', '/login', '/signup', '/verify', '/verify?email=nobody%40example.invalid', '/forgot-password', '/reset-password', '/security', '/privacy', '/terms', '/app/recovery?service=access-recovery']) {
  ...
  for (const [route, heading, label] of [
    ...
    ['/verify?email=nobody%40example.invalid', 'Check your email.', 'confirmation'],
