...
import { getService, type ServiceId } from './serviceCatalog';

/** Older browser-only walkthrough entries used the three general issue labels. */
type LegacyIssue = 'access' | 'transaction' | 'other';
export type Issue = ServiceId | LegacyIssue;
...
const legacyIssueNames: Record<LegacyIssue, string> = { access: 'Wallet access', transaction: 'Transaction question', other: 'Other issue' };
export function activityIssueName(issue: Issue): string {
  return getService(issue)?.name ?? legacyIssueNames[issue as LegacyIssue] ?? 'Walkthrough';
}
